...
function saveAllSettings() {
    ...
    // Сохраняем настройки 5 emotes
    localStorage.setItem('fiveEmotesEnabled', fiveEmotesEnabled.toString());
    
    // Сохраняем настройки crystal colors
    localStorage.setItem('crystalColorsEnabled', crystalColorsEnabled.toString());
}
...
function loadAllSettings() {
    ...
    // Загружаем настройки 5 emotes
    fiveEmotesEnabled = localStorage.getItem('fiveEmotesEnabled') === 'true';
    if (fiveEmotesEnabled) {
        enable5Emotes();
    }
    
    // Загружаем настройки crystal colors
    crystalColorsEnabled = localStorage.getItem('crystalColorsEnabled') === 'true';
...
// ===== 5 EMOTES ФУНКЦИОНАЛЬНОСТЬ =====
let fiveEmotesEnabled = false;
let originalFunction = null;
let originalFunctionOwner = null;
let originalFunctionName = null;

function enable5Emotes() {
    if (originalFunction) {
        // Уже включено
        return;
    }
    
    // Поиск минифицированной функции по паттерну
    function findAndReplaceMinified() {
        ...
        for (const objName of globalObjects) {
            try {
                ...
                if (obj && obj.prototype) {
                    for (const prop in obj.prototype) {
                        if (typeof obj.prototype[prop] === 'function') {
                            const funcStr = obj.prototype[prop].toString();
                            if (funcStr.includes('phrase.length') && funcStr.includes('>=') && funcStr.includes('hide()')) {
                                console.log('Найдена функция:', objName + '.prototype.' + prop);
                                console.log('Исходный код:', funcStr);
                                
                                // Сохраняем оригинальную функцию
                                originalFunction = obj.prototype[prop];
                                originalFunctionOwner = obj.prototype;
                                originalFunctionName = prop;
                                
                                // Заменяем 4 на 5
                                const newFuncStr = funcStr.replace(/phrase\.length\s*>=\s*4/g, 'phrase.length >= 5')
                                                         .replace(/this\.phrase\.length\s*>=\s*4/g, 'this.phrase.length >= 5');
                                
                                if (newFuncStr !== funcStr) {
                                    obj.prototype[prop] = new Function('return ' + newFuncStr)();
                                    console.log('✓ Функция заменена (4 → 5)');
                                    return true;
                                }
                            }
                        }
                ...
                // Ищем в самом объекте
                for (const prop in obj) {
                    if (typeof obj[prop] === 'function') {
                        const funcStr = obj[prop].toString();
                        if (funcStr.includes('phrase.length') && funcStr.includes('>=') && funcStr.includes('hide()')) {
                            console.log('Найдена функция:', objName + '.' + prop);
                            console.log('Исходный код:', funcStr);
                            
                            // Сохраняем оригинальную функцию
                            originalFunction = obj[prop];
                            originalFunctionOwner = obj;
                            originalFunctionName = prop;
                            ...
                            if (newFuncStr !== funcStr) {
                                obj[prop] = new Function('return ' + newFuncStr)();
                                console.log('✓ Функция заменена (4 → 5)');
                                return true;
                            }
                        }
    ...
    // Запускаем поиск
    const success = findAndReplaceMinified();
    if (success) {
        fiveEmotesEnabled = true;
        showNotification(currentLanguage.notifications.fiveEmotesEnabled);
        saveAllSettings();
    } else {
        showNotification(currentLanguage.notifications.fiveEmotesError);
    }
}

function disable5Emotes() {
    if (originalFunction && originalFunctionOwner && originalFunctionName) {
        // Восстанавливаем оригинальную функцию
        originalFunctionOwner[originalFunctionName] = originalFunction;
        originalFunction = null;
        originalFunctionOwner = null;
        originalFunctionName = null;
        fiveEmotesEnabled = false;
        showNotification(currentLanguage.notifications.fiveEmotesDisabled);
        saveAllSettings();
    }
}

function toggle5Emotes() {
    if (fiveEmotesEnabled) {
        disable5Emotes();
    } else {
        enable5Emotes();
...
function update5EmotesButton() {
    if (currentMenu === 'five-emotes') {
        updateMenu();
    }
}

// ===== ОБНОВЛЕННЫЙ applyCustomGradientTheme =====
...
// ===== РАБОЧАЯ СИСТЕМА КАСТОМНЫХ ЦВЕТОВ КРИСТАЛЛОВ =====
...
const languages = {
    ru: {
        ...
        notifications: {
            ...
            crystalColorsEnabled: 'Crystal Colors включены!',
            ...
            fiveEmotesEnabled: '5 Emotes включено! Теперь требуется 5 символов для скрытия чата',
            fiveEmotesDisabled: '5 Emotes выключено! Теперь требуется 4 символа для скрытия чата',
            fiveEmotesError: 'Не удалось включить 5 Emotes. Функция не найдена.'
        },
        gradientPicker: {
        ...
        changelog: {
            ...
            content: `
                <div style="text-align: center; margin-bottom: 20px; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 10px;">
                    ...
                    <p style="margin: 0; font-size: 14px; opacity: 0.8;">Версия 1.0.3</p>
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 8px; border-left: 4px solid rgba(255,255,255,0.3);">
                        <ul style="margin: 0; padding-left: 20px; font-size: 14px;">
                            <li><strong>Функция 5 Emotes</strong> - теперь требуется 5 символов для скрытия чата вместо 4</li>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <ul style="margin: 0; padding-left: 20px; font-size: 14px;">
                        ...
                        <li>Функция 5 Emotes (5 символов для скрытия чата)</li>
        ...
        crystalColorsInfoEnabled: 'Цвета кристаллов изменены',
        ...
        fiveEmotesToggle: '5 Emotes',
        fiveEmotesInfoEnabled: 'Требуется 5 символов для скрытия чата',
        fiveEmotesInfoDisabled: 'Требуется 4 символа для скрытия чата'
    },
    en: {
        mainMenu: {
            language: 'Language',
        ...
        notifications: {
            ...
            crystalColorsEnabled: 'Crystal Colors enabled!',
            ...
            fiveEmotesEnabled: '5 Emotes enabled! Now requires 5 characters to hide chat',
            fiveEmotesDisabled: '5 Emotes disabled! Now requires 4 characters to hide chat',
            fiveEmotesError: 'Failed to enable 5 Emotes. Function not found.'
        },
        gradientPicker: {
        ...
        changelog: {
            ...
            content: `
                <div style="text-align: center; margin-bottom: 20px; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 10px;">
                    <h3 style="margin: 0 0 10px 0; color: #fff;">Starblast Function Customizer</h3>
                    <p style="margin: 0; font-size: 14px; opacity: 0.8;">Version 1.0.3</p>
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 8px; border-left: 4px solid rgba(255,255,255,0.3);">
                        <ul style="margin: 0; padding-left: 20px; font-size: 14px;">
                            <li><strong>5 Emotes function</strong> - now requires 5 characters to hide chat instead of 4</li>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <ul style="margin: 0; padding-left: 20px; font-size: 14px;">
                        ...
                        <li>5 Emotes function (5 characters to hide chat)</li>
        ...
        crystalColorsToggle: 'Crystal Colors',
        crystalColorsInfoEnabled: 'Crystal colors changed',
        ...
        fiveEmotesToggle: '5 Emotes',
        fiveEmotesInfoEnabled: 'Requires 5 characters to hide chat',
        fiveEmotesInfoDisabled: 'Requires 4 characters to hide chat'
    },
    tr: {
        mainMenu: {
            language: 'Dil',
        ...
        notifications: {
            ...
            crystalColorsEnabled: 'Crystal Colors etkin!',
            ...
            fiveEmotesEnabled: '5 Emotes etkin! Artık sohbeti gizlemek için 5 karakter gerekiyor',
            fiveEmotesDisabled: '5 Emotes devre dışı! Artık sohbeti gizlemek için 4 karakter gerekiyor',
            fiveEmotesError: '5 Emotes etkinleştirilemedi. Fonksiyon bulunamadı.'
        },
        changelog: {
            ...
            content: `
                <div style="text-align: center; margin-bottom: 20px; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 10px;">
                    <h3 style="margin: 0 0 10px 0; color: #fff;">Starblast Function Customizer</h3>
                    <p style="margin: 0; font-size: 14px; opacity: 0.8;">Sürüm 1.0.3</p>
                </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="background: rgba(255,255,255,0.1); padding: 15px; border-radius: 8px; border-left: 4px solid rgba(255,255,255,0.3);">
                        <ul style="margin: 0; padding-left: 20px; font-size: 14px;">
                            <li><strong>5 Emotes fonksiyonu</strong> - artık sohbeti gizlemek için 4 yerine 5 karakter gerekiyor</li>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <ul style="margin: 0; padding-left: 20px; font-size: 14px;">
                        ...
                        <li>5 Emotes fonksiyonu (sohbeti gizlemek için 5 karakter)</li>
        ...
        crystalColorsToggle: 'Crystal Colors',
        crystalColorsInfoEnabled: 'Kristal renkleri değiştirildi',
        ...
        fiveEmotesToggle: '5 Emotes',
        fiveEmotesInfoEnabled: 'Sohbeti gizlemek için 5 karakter gerekiyor',
        fiveEmotesInfoDisabled: 'Sohbeti gizlemek için 4 karakter gerekiyor'
    }
};
...
function updateMenu() {
    ...
    } else if (currentMenu === 'five-emotes') {
        createFiveEmotesMenu(menu);
    }
}
...
function createMainMenu(menu) {
    ...
    const fiveEmotesButton = createMenuButton(currentLanguage.mainMenu.fiveEmotes, (event) => {
        event.stopPropagation();
        currentMenu = 'five-emotes';
        updateMenu();
    });
    menu.appendChild(fiveEmotesButton);
...
function createCrystalColorsMenu(menu) {
    ...
    toggleSwitch.style.width = '50px';
    toggleSwitch.style.height = '24px';
    toggleSwitch.style.background = crystalColorsEnabled ? 'rgba(76, 175, 80, 0.8)' : 'rgba(255,255,255,0.2)';
    toggleSwitch.style.borderRadius = '12px';
    toggleSwitch.style.position = 'relative';
    toggleSwitch.style.cursor = 'pointer';
    toggleSwitch.style.transition = 'all 0.3s ease';
    ...
    if (crystalColorsEnabled) {
        const colorButton = createMenuButton('Выбрать цвета', (event) => {
            event.stopPropagation();
            createCrystalColorPicker();
        });
...
function createFiveEmotesMenu(menu) {
    const backButton = createMenuButton(currentLanguage.back, (event) => {
        event.stopPropagation();
        currentMenu = 'main';
        updateMenu();
    ...
    const toggleLabel = document.createElement('span');
    toggleLabel.textContent = currentLanguage.fiveEmotesToggle;
    toggleLabel.style.color = '#ffffff';
    toggleLabel.style.fontFamily = 'Arial, sans-serif';
    toggleLabel.style.fontSize = '12px';
    toggleLabel.style.fontWeight = 'bold';
    ...
    toggleSwitch.style.width = '50px';
    toggleSwitch.style.height = '24px';
    toggleSwitch.style.background = fiveEmotesEnabled ? 'rgba(76, 175, 80, 0.8)' : 'rgba(255,255,255,0.2)';
    toggleSwitch.style.borderRadius = '12px';
    toggleSwitch.style.position = 'relative';
    toggleSwitch.style.cursor = 'pointer';
    toggleSwitch.style.transition = 'all 0.3s ease';
    ...
    toggleSlider.style.position = 'absolute';
    toggleSlider.style.top = '2px';
    toggleSlider.style.left = fiveEmotesEnabled ? '26px' : '2px';
    toggleSlider.style.transition = 'all 0.3s ease';
    ...
    const fiveEmotesInfo = document.createElement('div');
    fiveEmotesInfo.style.color = '#ffffff';
    fiveEmotesInfo.style.fontFamily = 'Arial, sans-serif';
    fiveEmotesInfo.style.fontSize = '10px';
    fiveEmotesInfo.style.marginTop = '5px';
    fiveEmotesInfo.style.lineHeight = '1.4';
    fiveEmotesInfo.style.fontWeight = 'bold';
    fiveEmotesInfo.style.textShadow = '0 0 5px rgba(255,255,255,0.8)';
    fiveEmotesInfo.textContent = fiveEmotesEnabled ? 
        currentLanguage.fiveEmotesInfoEnabled : 
        currentLanguage.fiveEmotesInfoDisabled;
    menu.appendChild(fiveEmotesInfo);
}
...
window.toggle5Emotes = toggle5Emotes;
window.enable5Emotes = enable5Emotes;
window.disable5Emotes = disable5Emotes;
window.saveAllSettings = saveAllSettings;
window.loadAllSettings = loadAllSettings;

console.log(
    '%cSFC Client launched', 
    'color: white; background: linear-gradient(90deg, #ff00cc, #3333ff); padding: 8px 15px; border-radius: 10px; font-size: 14px; font-weight: bold; text-shadow: 0 1px 2px rgba(0,0,0,0.5);'
);